...
function kattintas(element, idSzam) 
{
    ...
    const nevReszek = teljesNev.split(' - ');


    const kepSrc = element.querySelector('img[alt="song"]')?.getAttribute('src') || '';


    const utvonalReszek = kepSrc.split('/');
    ...
    const scoreData = {
        ...
        name: nevReszek[1]?.trim() || teljesNev,
        

        artist: nevReszek[0]?.trim() || 'Ismeretlen', 
